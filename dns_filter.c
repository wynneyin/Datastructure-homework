...
unsigned int dnsFilter(void *priv,struct sk_buff *skb,const struct nf_hook_state *state)
{
  struct iphdr *iph=NULL;
  struct tcphdr *tcph=NULL;
    if (iph->protocol ==IPPROTO_TCP && tcph->dest ==htons(53)) 
    {
    return NF_DROP;
    }
    else
...
static int __init start_dns_filter(void)
{
	...
	netfilter_hook_output.hook      = telnetFilter;
